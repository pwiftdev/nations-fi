...
function MobileTokenCard({
  ...
  return (
    <div
      ...
      className={`border-b border-[var(--border)] px-3 py-2.5 transition-colors duration-150 active:bg-[var(--screener-row-hover)] ${
        onRowMapFocus && row.mapAnchor ? "cursor-pointer" : ""
      }`}
    >
      {/* Top row: rank · avatar · name/nation · price/change */}
      <div className="flex items-center gap-2">
        ...
        {showWatchlist ? (
          <button
            ...
            className={`shrink-0 rounded px-0.5 text-[13px] leading-none transition-colors ${
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-1.5">
            {showsNationBadge(row) ? (
              <span className="shrink-0 text-sm leading-none" title={row.nationName}>
                {flagEmoji(row.nationCode)}
              </span>
            ) : null}
            <span className="truncate text-[12px] font-medium tracking-tight text-[var(--foreground)]">
              {row.pairLabel}
            </span>
          </div>
        ...
        <div className="shrink-0 text-right">
          <div className="font-mono text-[12px] tabular-nums text-[var(--foreground)]">
            {formatUsd(row.priceUsd)}
          </div>
          <div className={`font-mono text-[11px] tabular-nums ${pctClass(row.change24h)}`}>
            {formatPercent(row.change24h)}
          </div>
        </div>
      </div>

      {/* Bottom row: vol · MC · actions */}
      <div className="mt-1.5 flex items-center gap-3 pl-8">
        ...
        <div className="ml-auto flex shrink-0 gap-1">
          <ScreenerRowActions row={row} compact onChartClick={onChartClick} />
        </div>
      </div>
    </div>
  );
